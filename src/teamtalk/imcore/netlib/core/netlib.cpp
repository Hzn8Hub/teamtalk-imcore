...
int netlib_option(net_handle_t handle, int opt, void* value) {
  ...
  switch (opt) {
    case NETLIB_OPT_SET_CALLBACK:
      pSocket->SetCallback((callback_t)value);
      break;
    case NETLIB_OPT_SET_CALLBACK_DATA:
      pSocket->SetCallbackData(value);
      break;
    case NETLIB_OPT_GET_REMOTE_IP:
      *(std::string*)value = pSocket->GetRemoteIP();
      break;
    case NETLIB_OPT_GET_REMOTE_PORT:
      *(uint16_t*)value = pSocket->GetRemotePort();
      break;
    case NETLIB_OPT_GET_LOCAL_IP:
      *(std::string*)value = pSocket->GetLocalIP();
      break;
    case NETLIB_OPT_GET_LOCAL_PORT:
      *(uint16_t*)value = pSocket->GetLocalPort();
      break;
    case NETLIB_OPT_SET_SEND_BUF_SIZE:
      pSocket->SetSendBufSize(*(uint32_t*)value);
      break;
    case NETLIB_OPT_SET_RECV_BUF_SIZE:
      pSocket->SetRecvBufSize(*(uint32_t*)value);
      break;
  }
